...
main() {
    ...
    # 配置变量
    VERSION="$1"
    REMOTE_USER="lepureroot"
    REMOTE_HOST="192.168.10.78"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        echo "开始上传文件到服务器..."
        scp lepure-opportunity-images-${VERSION}.tar lepure-opportunity-configs-${VERSION}.tar.gz deploy/cleanup-and-redeploy-opportunity.sh ${REMOTE_USER}@${REMOTE_HOST}:~/
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "  chmod +x ~/cleanup-and-redeploy-opportunity.sh"
            echo "  ./cleanup-and-redeploy-opportunity.sh ${VERSION}"
        else
            echo "错误：文件上传失败"
            echo ""
            return 1
        ...
        echo ""
        echo "跳过上传。手动上传命令："
        echo "  scp lepure-opportunity-images-${VERSION}.tar lepure-opportunity-configs-${VERSION}.tar.gz deploy/cleanup-and-redeploy-opportunity.sh ${REMOTE_USER}@${REMOTE_HOST}:~/"
    fi
}
