...
from app.utils.db_utils import init_database_with_retry, get_session, log_pool_status
...
def main():
    ...
    if settings.CRON_SCHEDULE:
        logger.info(f"调度方式: Cron 定时任务")
        logger.info(f"执行时间: {settings.CRON_SCHEDULE}")
    else:
        logger.info(f"调度方式: 轮询")
    ...
    if settings.CRON_SCHEDULE:
        ...
        # 例如: "10:30,14:30" 表示每天 10:30 和 14:30 执行
        ...
        logger.info(f"调度器已启动，使用 Cron 定时任务")
    else:
        # 方案 2: 使用轮询间隔（兼容旧配置）
